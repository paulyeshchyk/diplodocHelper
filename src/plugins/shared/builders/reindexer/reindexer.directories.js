...
const { getSectionMetadata } = require('../../context/frontmatter/frontmatter.facade.section.metadata');
...
function reindexDirectory(dir, parentIndex = '', sortOrder = 'ascending', sortKind = 'nonIndexedBottom') {
    ...
    const sections = items.filter(
        item => item.isDirectory() && fs.existsSync(path.join(dir, item.name, FrontMatterFiles.INDEX_MD))
    );
