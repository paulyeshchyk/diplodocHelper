...
/**
 * @param {string} dir
 * @param {{ (htmlPath: any): void; (htmlPath: any): void; (arg0: string): void; }} callback
 */
...
function buildTitleMap(buildDir) {
  ...
  walkHtmlFiles(buildDir, (htmlPath) => {
    ...
    if (fileName === "index.html" && path.dirname(htmlPath) === buildDir)
      return;
 ...
 * @param {Map<any, any> | Iterable<readonly [PropertyKey, any]>} titleMap
 * @param {Config} config
 */
...
function runGeneration(buildDir) {
  ...
  console.log(
    `Конфигурация: buildDir=${buildDir}, separator="${CONFIG.separator}"`,
  );
